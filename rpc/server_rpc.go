...
func main() {
	pokedexRPC := new(PokedexRPC)

	pokemonServer := rpc.NewServer()
	errRegist := pokemonServer.RegisterName("Pokedex", pokedexRPC)
	if errRegist != nil {
		fmt.Println("Erro ao registrar métodos:", errRegist)
		os.Exit(1)
	}
